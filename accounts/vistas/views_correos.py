...
def enviar_correo(request):
	if request.method == 'POST':
		...
		destinatario = ['aranza.gutierrezm.ok@gmail.com']  # Cambia esto por la dirección de correo deseada
...
def enviar_cotizacion(request, pk, destinatarios):
	...
	email = EmailMessage(subject, message, 'proyectos.inade@icloud.com', destinatarios)
...
def confirmar_recepcion(request, pk):
	cotizacion = get_object_or_404(Cotizacion, id=pk)
	if request.method == 'POST':
		messages.success(request, 'Gracias por confirmar la recepción de la cotización.')
		# Aquí puedes realizar acciones adicionales como actualizar el estado de la cotización en la base de datos
		return redirect('confirmacion_recepcion')  # Redirigir a la página de inicio o donde prefieras
